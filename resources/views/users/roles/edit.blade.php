...
@section('content')
    <main class="min-h-screen w-full ml-60 bg-gray-200">
        <div class="w-full py-5 text-center bg-white shadow-sm">
            <h1 class="text-xl font-bold">{{ __('Editing role') }}</h1>
        </div>

        <div class="w-full h-full px-80 py-10 bg-gray-200">
            <div class="w-full bg-white p-8 shadow-md">
                <form action="{{ route('roles.update', compact('role')) }}" method="POST">
                    @csrf
					@method('PATCH')
                    ...
                    <div class="block">
                        ...
                        <input type="text" placeholder="{{ __('Role name') }}" name="name" value="{{ $role->name }}" class="w-full border-[1.5px] border-form-stroke
                    ...
                    <div class="mt-8">
                        ...
                        <select name="permissions[]" multiple class="w-full border-[1.5px] border-form-stroke rounded-lg py-3 px-5 font-medium text-body-color
                            ...
                            @foreach ($permissions as $permission)
                                <option value="{{ $permission->id }}" @selected($role->hasPermissionTo($permission->name))>{{ __($permission->name) }}</option>
                            @endforeach
                        </select>
                    </div>
