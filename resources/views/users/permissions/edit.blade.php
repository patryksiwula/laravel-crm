...
@section('content')
    <main class="min-h-screen w-full ml-60 bg-gray-200">
        <div class="w-full py-5 text-center bg-white shadow-sm">
            <h1 class="text-xl font-bold">{{ __('Editing permission') }}</h1>
        </div>

        <div class="w-full h-full px-80 py-10 bg-gray-200">
            <div class="w-full bg-white p-8 shadow-md">
                <form action="{{ route('permissions.update', compact('permission')) }}" method="POST">
                    @csrf
					@method('PATCH')
                    ...
                    <div class="block">
                        ...
                        <input type="text" placeholder="{{ __('Permission name') }}" name="name" value="{{ $permission->name }}" class="w-full border-[1.5px] border-form-stroke
