...
@section('content')
    <main class="min-h-screen w-full ml-60 bg-gray-200">
        <div class="w-full py-5 text-center bg-white shadow-sm">
            <h1 class="text-xl font-bold">{{ __('Editing product') }}</h1>
        </div>
            ...
            <div class="w-full bg-white p-8 shadow-md">
                <form action="{{ route('products.update', compact('product')) }}" method="POST">
                    ...
                    <div class="block">
                        ...
                        <input type="text" placeholder="{{ __('Product name') }}" name="name" value="{{ $product->name }}" class="w-full border-[1.5px] border-form-stroke
					...
					<div class="mt-8">
                        ...
                        <input type="number" placeholder="{{ __('Product quantity') }}" name="quantity" value="{{ $product->quantity }}" class="w-full border-[1.5px] border-form-stroke
                            rounded-lg py-3 px-5 font-medium text-body-color placeholder-body-color outline-none focus:border-primary
                            active:border-primary transition disabled:bg-[#F5F7FD] disabled:cursor-default">
					</div>
